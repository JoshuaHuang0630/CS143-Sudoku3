...
public class SudokuBoard
{
	...
	private boolean checkBox()
	{
		for (int boxRow = 0; boxRow < 3; boxRow++)
		{
			...
			{ // checks the first, second, and third box
				Set <Integer> set = new HashSet <>();
				for (int r = 0; r < 3; r++)
				{
					for (int c = 0; c < 3; c++)
					{ // checks the first row, then first column of the box
	...
	public boolean isSolved()
	{
		...
		for (int num = 1; num <= 9; num++)
		{
			...
			{ // this loop goes from 1-9 and checks if the map contains the number and the occurrences the number came up
				return false;
			}
		}
